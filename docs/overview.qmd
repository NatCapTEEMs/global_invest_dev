...
- The {{< meta n_services >}} modules produce {{< meta n_accounts >}} accounts, because three of them each value two services. We compute the value for {{< meta n_ours >}} of those accounts. {{< meta n_partly >}} build one part of the value ourselves and take the other part from a published source. {{< meta n_reads_a_value >}} read a published per-country value and rescale it if necessary. {{< meta n_not_wired >}} are waiting on a model run, named on their own slides.
