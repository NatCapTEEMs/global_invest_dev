...
| {{< meta n_reads_a_value >}} | We take the economics from a published table. Two of them turn a published rent share into dollars; three report a finished per-country figure. |
| {{< meta n_not_wired >}} | We value the output of a model we do not yet run ourselves. Three report a total; stormwater reports retained volume only, because its price is a placeholder of 1. |
...
- [Values hydropower rent and priced water withdrawal. Hydropower is computed from the CWoN wealth table for every country it values, {{< meta water_supply_total >}} across 97 countries, and reproduces the drive's committed output exactly on the 95 countries that output covers. Water use is computed in the library, agriculture {{< meta water_use_ag_total >}} across 126 countries and all-sector {{< meta water_use_all_total >}} across 154.]{.state}
