...
| the five accounting services | — | library-wide contract tests only (next line) — a gap this slide states rather than hides |
...
- What is missing, stated plainly: science-level tests for the five accounting services and landslide — each needs a small synthetic pin of its own arithmetic, the way erosion and recreation have.
...
- **Q1 (design, coastal team):** was EEZ-only chosen to avoid double-counting the land-side pixels with terrestrial carbon? The land side carries 97.5 percent of mangrove and 84 percent of salt marsh area, so the convention moves the total 4.5 times.
- **Q2 (inputs, coastal team):** which exact seagrass and salt marsh input versions produced the report's {{< meta coastal_report >}}? Our replication gives {{< meta coastal_eez >}} with mangrove matching exactly, and the report's own seagrass area (62.21 Mha) exceeds the published range it cites (16 to 26.7 Mha).
...
- **Q1 (landslide team): Can you export one machine-readable per-country output from a v0.2.0 run — a CSV is enough?** With it we run, diff, and the service reaches the verified bar.
- **Q2 (landslide team): Which input versions did the release-day run use?**
...
- **Q1 (fisheries team): Do you bless the RCP8.5-for-RCP7.0 substitution, or name a replacement?** The 8.5 series stands in because no 7.0 series exists — defensible, but currently discoverable rather than stated.
- **Q2 (fisheries team): Which CWoN vintage is behind the HAR file?** For the record and the methods writeup.
...
- One flagged question for the method writeup: the travel-cost distance multiplies the fuel cost (USD per km) by ring distance in the raster's native pixel units — we ported it faithfully and will resolve it against the reference output rather than guess.
...
- **EPA VSL:** landslide values mortality, so one more ask for its team — which VSL does v0.2.0 use?
