...
- **Q1 (owner): Which FAO items belong in the livestock list — and does milk?** The current 29 items are meats and eggs only, so India's captured value is 4 percent of its FAO livestock total.
- **Q2 (owner): Should livestock use CWON's pasture rent shares instead of crop's coefficients?** The module currently reads the crop coefficient file, a leftover of cloning.
- **Q3 (owner): Can you bless a reference output we can verify against once the country join is fixed?** The join currently drops countries that have both value and coefficient on file (India ends at zero); the fix is mechanical but changes numbers, and we will not trust the result without an anchor.
