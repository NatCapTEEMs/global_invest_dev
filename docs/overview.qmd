...
poll_with_sufficiency: "$163.46bn"
poll_fao: "$515bn"
livestock_ind: "$136bn"
...
- **Q1:** which definition does the account want? The crop output at stake if pollinators vanished, {{< meta poll_raster >}}, or the service habitat currently delivers, {{< meta poll_with_sufficiency >}}. Both are ours, on one grid at 2019 prices; the second multiplies by 300 m habitat sufficiency. Only the second responds to land-use change, which is what makes it a service rather than a property of the crop mix.
...
- [Where that leaves the comparison: ours is {{< meta erosion_total >}} against your {{< meta erosion_author_ses11 >}}, within about four percent, while China sits thirty percent higher in ours, India nearly thirty lower and the Philippines forty higher. A country's area is now the sum of the territories the boundary file splits it into, rather than any one of them, and it can move CHN, FRA, GBR, IND and TUR. It does not account for the country split above, and on the r250 boundary it cannot arise, since that file carries one row per country. The figures above come from a run that predates the change, and the rerun will restate them.]{.state}
