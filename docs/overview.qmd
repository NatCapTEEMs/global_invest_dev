...
- **Q1:** does the account want the baseline scenario or the expanded one? The methodology defines SES-11, severe erosion at 11 t/ha/yr and 2 for small-area or low-elevation countries, reported as {{< meta erosion_author_ses11 >}}, and EPS-5 relaxing that to 5, reported as {{< meta erosion_author_eps5 >}}. Our configuration is SES-11, so the run already matches the stated baseline, and the choice roughly doubles the number.
- **Q1b:** which of the three shock methods is the account's? Separately from the scenario, we compute damage (thresholded area share), service (threshold-free and magnitude-weighted per crop) and service-threshold (service restricted to a fixed severe-pixel set) side by side.
...
- **Q3:** did the reported run give a border cell to the country covering its centre, or to whichever country was rasterised last? Ours did the latter until this week, and it would explain why the two totals agree within about four percent while the countries do not: against the six largest reported, ours puts China thirty percent higher, India nearly thirty percent lower and the Philippines forty percent higher, and reorders the ranking.
