...
- A service that turns a density into a total multiplies by cell area, so the cell area silently scales every number it produces. The right answer differs by grid, and there is no single convention that fits all of them.
- Two strategies are already in use and both are correct. Erosion and ntfp work in equal-area projections, where a constant is exact and a latitude formula would be wrong. Pollination and the carbon services work in geographic coordinates and carry a per-latitude area from the shared pyramid.
- The two fail differently. A constant cannot be applied wrongly per cell. An area raster is more precise and every consumer has to remember it, which is how pollination's earlier figure went wrong.
- **Q:** should each service state which grid it is on and what its cell area rests on, the way the erosion code refuses to run on an unprojected input?
