...
crop_total: "$623bn"
crop_subsistence_total: "$18.32bn"
...
- **Q2:** is subsistence in scope as a separate crop subgroup? Adding it would double-count, because FAOSTAT's production values already include part of what households grow and eat themselves. The reference pipeline is now ported. Its units disagree with its sources (cropland in thousands of hectares against an intensity per hectare), so we publish {{< meta crop_subsistence_total >}} across 66 countries and its own arithmetic, {{< meta crop_subsistence_reference >}}, beside it as proof the port is faithful.
