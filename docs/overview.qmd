...
|---|---|
| {{< meta n_ours >}} | We compute it. The quantity, the price and nature's share are all built in the repo. Twelve of the thirteen report a total; erosion's run is still going, so it has none yet. |
...
- The ladder measures work, not confidence, and reading it as a quality ranking gets it backwards. The two accounts without a usable number sit at the top: stormwater has no dollar total because its price is a placeholder, and erosion has no number until its run finishes. Crop production, down at table arithmetic, reproduces its reference exactly.
...
- [Values soil the land cover keeps in place: prevented erosion mapped by running the sediment-delivery model, valued through crop productivity. It is running for the first time, and we report no number until it lands.]{.state}
...
- For a country id the rule is settled. A cell goes to whichever country covers its exact centre, so it has exactly one owner. The alternative, all-touched, hands a border cell to every country whose outline touches it, and that cell's forest is then added to two national totals, so the world sums to more than it holds.
- **Q1:** do we adopt the centre rule for pollination and terrestrial_carbon? Both aggregate through the shared helper, which is all-touched, so they double-count their border cells today, and adopting it moves two published totals.
