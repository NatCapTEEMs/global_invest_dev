...
crop_total: "$623bn"
crop_subsistence_total: "$1.83bn"
crop_subsistence_corrected: "$18.32bn"
...
- **Q2:** is subsistence in scope as a separate crop subgroup? Adding it would double-count, because FAOSTAT's production values already include part of what households grow and eat themselves. The reference pipeline is now ported and reproduces exactly, at {{< meta crop_subsistence_total >}} across 66 countries. ⚠ Its units disagree with its sources (cropland in thousands of hectares against an intensity per hectare), so the same calculation reads {{< meta crop_subsistence_corrected >}}.
