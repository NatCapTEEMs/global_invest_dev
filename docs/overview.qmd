...
- **Q2:** should the erosion shock be recomputed per scenario rather than read from the frozen dependency table? The four tasks that would do it are written -- the sediment model, the upstream routing, the exposure step and the shock -- but dynamic_es is never set by any consumer, so every run today reads the frozen table instead.
