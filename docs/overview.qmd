...
- **Q3:** should timber take its ecosystem share from GTAP when the other services take one from CWoN? Following Damania et al. (2023) it multiplies gross returns by the GTAP 10 land factor share for each of the 18 agro-ecological zones, so the share does vary by zone rather than being one economy-wide number. CWoN publishes a forest rent that would answer the same question in the same currency as the rest of the account, so this is a consistency choice rather than a data gap.
