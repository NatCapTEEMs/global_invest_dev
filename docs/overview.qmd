...
poll_raster: "$388.90bn"
poll_centre_delta: "-0.12%"
poll_centre: "$388.43bn"
poll_fao: "$515bn"
livestock_ind: "$136bn"
...
- **Q1:** what attribution step turns water use into a nature share? The all-sector figure is the gross economic value of water used, close to half of world GDP.
...
- **Q3:** do we move pollination to the centre rule for country totals? Measured on our own raster, the world total moves {{< meta poll_centre_delta >}}, from {{< meta poll_raster >}} to {{< meta poll_centre >}}, but 96 of 208 countries move by more than 5 percent and Iraq by 66, losing two thirds of its value. Pollination is on a 5.5 km grid while the country boundaries are drawn on a 10 arc-second one, so a cell claimed by touch reaches a long way across a border.
...
- **Q2:** which erosion shock did the NGFS runs actually use? The tracked scenario templates all name erosion as dynamic, so it should be recomputed per scenario. Four derived input copies disagree with them and with each other, and one carries no dynamic_es column at all, in which case the run silently reads the frozen dependency table instead.
