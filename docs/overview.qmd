...
|---|---|
| {{< meta n_ours >}} | We compute it. The quantity, the price and nature's share are all built in the repo. Eleven of the twelve report a total; erosion's run is still going, so it has none yet. |
| {{< meta n_partly >}} | We compute part of it. One component is ours and the other is read from a published source. |
| {{< meta n_reads_a_value >}} | We take the economics from a published table. Two turn a published rent share into dollars; two report a finished per-country figure. |
