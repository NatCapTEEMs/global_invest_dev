...
- [A border cell now goes to the country covering its centre. Under all-touched, which the author's code sets and ours did too, every country whose outline touches a cell claims it and the one rasterised last keeps it, so a country's value moved with its row order in the boundary file. The move from ee_r264_correspondence to ee_r250 changes nothing, measured: r250 is exactly the dissolve of r264 and the rasteriser already keyed on ISO3.]{.state}
- [Ours is {{< meta erosion_total >}} against the author's {{< meta erosion_author_ses11 >}}, within about four percent, while China sits thirty percent higher in ours, India nearly thirty lower and the Philippines forty higher. A country's area is now summed over the territories the boundary file splits it into rather than taken from any one, which can move CHN, FRA, GBR, IND and TUR. On the r250 boundary it cannot arise at all, since that file carries one row per country. The figures above predate the change and the rerun restates them.]{.state}
