...
- **Q4:** should a habitat be credited with a cell its mapped extent only clips? Crediting every touched cell whole put the mangrove extent at roughly twice the area of the polygons themselves, and salt marsh near three times, because both are ribbons about as wide as a cell. Measuring the covered fraction instead brings mangrove back to the mapped extent, which independently matches Global Mangrove Watch's published figure. Both the EEZ-only and all-rows totals in Q1 sit on top of the inflated extent.
...
- **Q2:** does your February SDR run supersede the December one? We have both: our own InVEST SDR run from 16 August produced usle_2019_revised_dec_14, and the account reads your usle_2019_revised_feb_13 pulled from your drive. We would rather read the current one knowingly than by whichever suffix the config happens to name.
- **Q1:** should the EPS-5 sensitivity be reported beside the baseline? Nothing to decide on the baseline itself: the manuscript says results correspond to SES-11 unless otherwise stated, and our thresholds are already 11 t/ha/yr with 2 for small-area and low-elevation countries, with nothing overriding them. EPS-5 relaxes the threshold to 5 as a sensitivity analysis rather than an alternative, and it takes the reported figure from {{< meta erosion_author_ses11 >}} to {{< meta erosion_author_eps5 >}}, so whether the account carries it is worth settling.
- [One change worth telling you about: a border cell now goes to the country covering its centre. Under all-touched, which your code sets and ours did too, every country whose outline touches a cell claims it and the one rasterised last keeps it, so a country's value moved with its row order in the boundary file. We also moved from ee_r264_correspondence to ee_r250, then measured it and it changes nothing, since r250 is exactly the dissolve of r264 and the rasteriser already keyed on ISO3.]{.state}
- [Where that leaves the comparison: ours is {{< meta erosion_total >}} against your {{< meta erosion_author_ses11 >}}, within about four percent, while China sits thirty percent higher in ours, India nearly thirty lower and the Philippines forty higher. That was our bug, not a difference with you, and it is fixed. Because the boundary file splits thirteen countries into territories, we were taking one territory's area as the country's, so China and India qualified as small and drew the lower soil-loss tolerance, which counts far more of their cropland as severely eroding. Seven countries moved on the fix. The figures above come from a run that predates it, and the rerun will restate them.]{.state}
...
- **Q3:** is the agriculture gap the missing water-stress attribution? Your Submission_WaterUse_Agriculture appendix defines a lambda per crop and country, the FAO yield response coefficients per growth stage weighted by the ratio of actual to maximum evapotranspiration, with production value split rainfed and irrigated by SPAM 2020. We do not implement it: our water use is withdrawal times AQUASTAT efficiency, which is the gross value of water used rather than the part attributable to ecosystems. That would explain why the agriculture pair is {{< meta water_use_ag_total >}} against your $554.5bn while the all-sector pair, which has no attribution on either side, agrees closely. Is that the whole of the difference, and should we implement the lambda?
...
- **Q3:** are the JRC depth-damage curves meant to be applied over true ground area? They are a damage per square metre of asset, and the damage pass gives every cell the same area, taken from a projection whose cells shrink away from the equator. Weighting each country's damage by its latitude moves the global total by about a third, and the countries carrying most of the value sit far from the equator. The treatment is the pipeline's rather than ours, which is why our recompute reproduces its table.
...
- The method is written up in `timber_provision_appendix`, now staged at `base_data/global_invest/timber_provision/reference/` so the claims below can be checked without Drive. It is the only description of the value raster we have: the repo the account cites for timber holds carbon, crop, livestock, pollination and renewable energy, and no timber code.
- **Q2:** where does the value raster stop being per hectare and become per cell? The appendix gives every input per hectare, yield in cubic metres per hectare times a price per cubic metre times a dimensionless land share, and the account sums the raster flat on a geographic grid. Summing a per-hectare quantity that way would be wrong, and the magnitudes say it is not happening: flat comes to about $42 per hectare per year of managed forest, an ordinary forest land rent, where treating it as a density gives $337 and a world total above the whole forest-products industry. So the conversion is there and undocumented, and we would like to point at it.
- **Q3:** should timber take its ecosystem share from GTAP when every other service takes one from CWoN? Timber multiplies gross returns by the GTAP 10 land factor share for each of the 18 agro-ecological zones, following Damania et al. (2023), so the share does vary by zone. Every other service uses a CWoN rent. Both name the same thing, the part of gross output that accrues to land rather than to labour and capital, and they are built differently and come from different sources. The account adds the services together, so that share ought to mean one thing across them. Nobody chose this: it arrived with the study timber was ported from.
- **Q4:** can we have the CWoN timber rent? It exists. CWoN 2024 carries timber as a land asset category beside agricultural land, forest recreation, non-wood forest products and forest water services, and its timber wealth per capita fell 28 percent between 1995 and 2020. We just do not hold the file: what is staged covers crop, livestock, fisheries, renewable energy, extractive energy and non-wood forest products, and non-wood is a different quantity. Worth knowing that the two approaches are closer than the source difference suggests, because the SNA and SEEA-CF count forest timber as an asset only where it can be logged commercially at a profit, which is the same restriction as the forest-management mask we are missing.
...
- **Q2:** who multiplied three countries by five, and why? The rate file we read, `nontimber_price_iucn_edited.csv`, is identical to CWoN for 3,045 of its 3,108 country-year pairs and exactly five times CWoN for the other 63, which are Central African Republic, DR Congo and Mozambique in every one of 21 years. Surgical rather than corrupt, so somebody had a reason, and all three are Congo Basin or Miombo countries where most non-timber use never reaches a market, which would be one. Those three come to {{< meta ntfp_edit_three >}} of {{< meta ntfp_total >}}, DR Congo alone {{< meta ntfp_edit_cod >}}, and without the edit the service is {{< meta ntfp_no_edit >}}.
...
- Eleven service entries rest on reproducing an author's committed output. Where that output came from the code we ported, the two sides share every assumption, so agreement establishes that the port is faithful and cannot establish that the number is right.
- Both services anyone has examined this way turned out to carry an assumption inherited from the source rather than introduced by us. Pollination summed a raster its own author documents as a density. Flood control measures every cell as the same size on a projection whose cells shrink away from the equator.
- **Q:** what should a service show beyond agreement with its author, before the account treats its number as verified?
- **Q:** when a check finds something, it is a correction to the author's published number rather than to our port. Who takes it to them, and does the account wait for their answer?
