...
moisture_recycling_gep: "$127.7bn"
moisture_recycling_export: "$75.3bn"
...
recreation_total: "$192.36bn"
recreation_total_degrees: "$1.73bn"
fire_cause_total: "-$1.3bn"
fire_baseline_total: "-$22.4bn"
fire_area_total: "+$16.7bn"
renewable_wind: "$112.52bn"
renewable_solar: "$54.41bn"
renewable_geothermal: "$6.56bn"
stormwater_volume: "185.85bn m3"
groundwater_recharge_gep: "$7.21M"
pest_control_gep: "$10.34bn"
pest_control_gep_no_premium: "$7.49bn"
health_hypertension_gep: "$148.71bn"
health_hypertension_appendix: "$104.66bn"
health_hypertension_extended: "$247.44bn"
health_depression_gep: "$97.96bn"
health_depression_author: "$159.6M"
...
- Every module whose run has finished has rendered its report.
...
| Table arithmetic | {{< meta cat_table >}} | crop, livestock, both fisheries, energy, hydropower, water use, air filtration, sandstorm, wildfires, water quality, coastal protection, moisture recycling, groundwater recharge, pest control |
| Raster arithmetic | {{< meta cat_raster >}} | terrestrial carbon, coastal carbon, pollination, ntfp, timber, floods, stormwater, both health strands |
...
| landslide_mitigation | {{< meta landslide_total >}} ({{< meta landslide_deaths >}} avoided deaths) | no. The all-services sheet's column ($865M) is the first per-country reference, and it disagrees with ours everywhere: unrelated country splits under near-matching totals |
...
| recreation | {{< meta recreation_total >}} with the travel distance in kilometres | no. The author's table is not reachable, and the sheet's {{< meta recreation_total_degrees >}} is the degree construction rounded to three significant figures |
...
| moisture_recycling | {{< meta moisture_recycling_gep >}}, of which {{< meta moisture_recycling_export >}} crosses a border as exported rain | no. No reference figure exists anywhere; the construction implements the submission's own valuation direction |
| groundwater_recharge | {{< meta groundwater_recharge_gep >}} avoided pumping cost | yes. The sheet's column reproduced to the cent in all 108 valued countries |
| pest_control | {{< meta pest_control_gep >}} with the appendix's organic price premium, {{< meta pest_control_gep_no_premium >}} without | partly. The sheet's $6.25bn sits at the no-premium level, log-correlation 0.93 across countries; its own pipeline is not on the drive |
| health_hypertension | {{< meta health_hypertension_gep >}} over the 20 countries with a cost study | partly. Rank yes (log-correlation 0.989 on the appendix's {{< meta health_hypertension_appendix >}}), level 42 percent above on named deviations; no pipeline exists on the drive |
| health_depression | {{< meta health_depression_gep >}} at the method's stated scope | no, deliberately. The author's {{< meta health_depression_author >}} is the same construction at roughly 1/400th of the urban population, so the gap measures the undocumented run scope |
...
- [Values visits to natural recreation sites at travel cost: 1 km sites ranked by land-cover shares, protected areas and road access, then a gravity model sends residents on day trips and tourists on overnight stays into the high-quality sites. Computed end to end in the library, with the travel distance converted to the kilometres the fuel cost prices: {{< meta recreation_total >}}. The source construction multiplied the per-km cost by a distance in degrees, giving {{< meta recreation_total_degrees >}} -- the level the sheet carries -- and the value is exactly linear in the conversion, so the two differ by the 111.32 km in an equatorial degree.]{.state}
- **Q1:** where can we find the source pipeline's results_by_country.csv?
- **Q2:** did the travel-cost step intend kilometres? The fuel-cost column is USD per km, so we now convert the degree pixel size at 111.32 km per degree -- and since the answer moves the row a hundredfold, it needs the author's word, not just ours.
...
- [Values the rain a country's land sends to its own and other countries' water use. The WAM2layers country-to-country matrix says what fraction of each country's precipitation evaporated from each other country's land; each destination's irrigation and domestic water GEP is scaled by its terrestrial-origin share and credited upstream to the source countries, giving {{< meta moisture_recycling_gep >}}, of which {{< meta moisture_recycling_export >}} crosses a border. A re-attribution of the water-supply row, never an addition to it.]{.state}
- **Q1:** how much terrestrial evaporation is the ecosystem's? The run counts all of it; the literature's transpiration fraction would roughly halve the row.
- **Q2:** how should the account carry it -- as a decomposition of water supply, or as a row holding only the transboundary part? Added whole to water supply it would count the same water twice.
- **Q3:** should hydropower's water value be re-attributed too? Rain feeds the rivers; the current construction re-attributes irrigation and domestic only.

## groundwater_recharge

- [Values the pumping energy recharge saves: a higher water table means every cubic metre withdrawn is lifted a shorter distance, priced at the country's electricity tariff. The sheet's column is reproduced to the cent in all 108 valued countries, {{< meta groundwater_recharge_gep >}} in total. The magnitude is the method's own: one year's recharge is one year's marginal head increment, a fraction of a cent per cubic metre.]{.state}
- **Q1:** is the service in scope? The sheet's note argues recharge is natural-capital value whose end products are counted elsewhere, yet the sheet carries the column. Avoided pumping energy does not obviously overlap purification or the irrigation premium, so the double-count argument is weaker than the note assumes.
- **Q2:** if in, which construction -- one year's marginal head increment ({{< meta groundwater_recharge_gep >}}), or the stock of avoided water-table depth, which the owner's staged groundwater-depth data could support and which would be two to three orders of magnitude larger?

## pest_control

- [Values the extra yield natural enemies provide on organically managed cropland, at the organic producer price -- organic only by design, and the full value attributed to nature with no lambda. Built from the appendix's own sources: FiBL areas, FAOSTAT yields and prices, the Boldorini effect sizes and the appendix's yield ratios, giving {{< meta pest_control_gep >}} with the appendix's 38 percent organic price premium and {{< meta pest_control_gep_no_premium >}} without it. The committed column's $6.25bn sits at the no-premium level (log-correlation 0.93 across countries), and its pipeline is not on the drive.]{.state}
- **Q1:** does the committed $6.25bn include the organic price premium the appendix describes? The level says no; with it the same construction gives {{< meta pest_control_gep >}}.
- **Q2:** what produced Viet Nam's $291M? The staged FiBL 2019 areas support roughly a ninth of it under any reading of the method.
- **Q3:** can the final pipeline and its inputs be shared, so the row stops being a carried-across file?

## health_hypertension

- [Values the hypertension treatment cost urban greenspace avoids: per urban pixel, WHO prevalence times the NDVI-driven risk reduction times the population aged 30-79, priced at the country's published cost per case. Built here end to end from the appendix's sources ({{< meta health_hypertension_gep >}} over the 20 countries with a cost study); the appendix's own table is {{< meta health_hypertension_appendix >}} over 19, and our run matches its ranking at 0.989 while sitting 42 percent above its level on the named deviations (1 km NDVI against their 300 m, the wider SMOD urban mask).]{.state}
- **Q1:** the drive's committed per-country CSV mis-attributes 7 of its 19 countries -- the values, sorted by country name, were pasted against labels sorted alphabetically, so Nigeria shows Poland's $6.86bn. Can it be re-exported from the appendix's Table 6?
- **Q2:** which vintage is the sheet's $106.65bn over 13 countries? It matches neither the appendix nor the committed CSV, and lacks six of the appendix's countries.
- **Q3:** coverage is the price table, not the method -- 20 countries have refereed treatment costs. Transferring costs on the ln-ln fit against GDP per capita (R2 0.32, observed studies kept) extends the row to {{< meta health_hypertension_extended >}} over 187 countries: is that the account's route to full coverage, or do the authors' own extrapolated costs replace it?

## health_depression

- [Values the depression cases urban greenspace prevents, at the societal cost of a case, following the InVEST urban mental health model. Built here end to end at the method's stated scope -- all urban residents of the 32 countries with a cost study -- giving {{< meta health_depression_gep >}} and 64.5M prevented cases; the author's file carries {{< meta health_depression_author >}}, the same construction at roughly 1/400th of that population, so the run's undocumented spatial extent is now a measured question rather than a guess.]{.state}
- **Q1:** what extent did the author's run cover? Our full-scope build is 370 times the file at the median country (USA 835, Germany 405), which says a city sample.
- **Q2:** which of the drive's two variants is blessed -- `high resolution` ({{< meta health_depression_author >}}, the sheet's column) or `low resolution` ($167.3M)?
- **Q3:** the costs are USD PPP, mostly 2018, against the account's 2019 USD -- restate, or carry the caveat?
- **Q4:** the two health strands share the same greenness and comorbid outcomes -- how does the account avoid double counting between them?
