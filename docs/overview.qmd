...
- initialize_paths resolves the science inputs a service needs, in one place: the code names what it needs relative to the shared base data, and each machine finds its own copy.
...
- **Q4 (room, architecture): Is fisheries in the right family?** Today the pipeline groups it with the nature channels — it enters the cc_es pass alongside carbon, pollination and erosion, and the nature-off baseline carries no fisheries shock. But unlike those three, its shock is RCP-driven and reads no land-use map, which is the signature of the climate-damage family (crop yield, labor productivity — present in every world including the baseline). We are keeping today's grouping; we want the room to confirm it is the intended one, because moving it would change the decomposition.
...
- **The room:** pollination's definition, the shoreline convention, crop's units, the map dissolve, base year 2019, subsistence scope, marine carbon's fate, fisheries' family (nature channel vs climate damage).
