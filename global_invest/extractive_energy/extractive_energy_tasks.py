"""Extractive-energy GEP tasks: the three committed fuel outputs on the r250 rows."""
import os
...
def gep_calculation(p):
    ...
    gas = pd.read_csv(p.extractive_energy_gas_path)
    coal = pd.read_csv(p.extractive_energy_coal_path)
    oil = pd.read_csv(p.extractive_energy_oil_path)
    ...
    df_gep = xe.extractive_energy_gep_by_country(gas, coal, oil, countries)
