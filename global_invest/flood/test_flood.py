"""Unit tests for the flood port (committed-output anchored).
...
The calculation section pins the ported per-step science (flood_functions) on hand-built
...
from global_invest.flood import flood_functions as fc
...
def test_join_keeps_all_countries_and_distinguishes_zero_from_unassessed():
    ...
    out = fc.flood_gep_by_country(avoided, countries)
    assert len(out) == 250
    assert (out['flood_gep'] == 0).sum() > 0        # assessed, zero avoided damage
    # ESH (Western Sahara) was never assessed: NaN, not zero.
    assert np.isnan(out.set_index('iso3_r250_label').loc['ESH', 'flood_gep'])


def test_es_config_and_parameters_rows_hydrate_flood(tmp_path):
    from global_invest import utilities
    p = SimpleNamespace()
    p.input_dir = str(tmp_path / 'input')
    p.get_path = lambda *a, **k: '/resolved/' + '/'.join(a)
    utilities.hydrate_es_config(p, 'flood', log=lambda *a: None)
    assert p.gep_base_year == 2019
    utilities.hydrate_es_parameters(p, 'flood', log=lambda *a: None)
    assert p.flood_avoided_damage_path.endswith('country_avoided_damage_usd2019.csv')
