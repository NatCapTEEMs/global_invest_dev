...
if __name__ == '__main__':
    ...
    p.project_name = 'gep_coastal_carbon'  # Determines the folder created to store intermediate and final results.
    ...
    # Task tree
    coastal_carbon_initialization.build_gep_service_task_tree(p) # Defines the actual logic of the model. Navigate into here to see what the model does.

    # Project level attributes
    p.df_countries_marine_csv_path = p.get_path('cartographic', 'ee', 'eemarine_r566_correspondence.csv') # ProjectFlow downloads all files automatically via the p.get_path() function.
    p.gdf_countries_marine_vector_path = p.get_path('cartographic', 'ee', 'eemarine_r566_correspondence.gpkg')
    p.mangrove_vector_path = os.path.join(p.base_data_dir, 'coastal_carbon', 'gmw_v3_2019_vec', 'gmw_v3_2019_vec.shp')
    p.ha_per_cell_1sec_path = p.get_path('pyramids', "ha_per_cell_1sec.tif")
    ...
    p.carbon_prices_path = os.path.join(p.base_data_dir, 'coastal_carbon', 'carbon_prices.xlsx')
