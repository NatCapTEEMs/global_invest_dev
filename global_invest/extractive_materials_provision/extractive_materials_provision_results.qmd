...
for k, v in p.results[service_label].items():
    ...
    if k == 'gep_by_country_base_year':
        total_agricultural_value = df['extractive_materials_provision_gep'].sum()
    ...
    if k == 'gep_by_year':
        p.gep_by_year_table = df[['year', 'extractive_materials_provision_gep']]
        # Drop values that are zero
        p.gep_by_year_table = p.gep_by_year_table[p.gep_by_year_table['extractive_materials_provision_gep'] > 0]
        ...
        plt.plot(p.gep_by_year_table['year'], p.gep_by_year_table['extractive_materials_provision_gep'], marker='o')
