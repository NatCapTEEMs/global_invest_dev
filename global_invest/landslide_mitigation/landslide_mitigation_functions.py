...
def parse_gpd_grid_definition(gpd_text):
    """Parse an NSIDC .gpd grid parameter definition file into a dict.

    .gpd format is `Key: value ; comment` lines. Only pulls what this pipeline needs. Takes the
    file's text rather than its path, so the parsing can be tested on a string.
    """
    params = {}
    for line in gpd_text.splitlines():
