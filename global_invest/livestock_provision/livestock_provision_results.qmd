...
p.project_dir = os.environ.get('PROJECTFLOW_ROOT', local_project_dir) # Set the project directory. This is where all intermediate and final results will be stored.
...
Livestock production attributable to ecosystem-provided feed is valued at $`{python} f"{total_agricultural_value:,.2f}"` in the base year. The attribution currently uses a land rental rate standing in for the feed share, so the number is provisional. The tables below aggregate it by income group, region and continent.
