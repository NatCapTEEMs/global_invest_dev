...
p.base_data_dir = "G:/Shared drives/NatCapTEEMs/Files/base_data/submission" # Set where data outside the project will be stored. CAUTION: For GEP we are using the shared Google Drive, but best practice is to use a local directory that you can control (also it's faster)
...
for k, v in p.results[service_label].items():
    ...
    if k == 'gep_by_country_base_year':
        ...
        if 'income_grp' in df.columns:
            ...
            p.gep_by_income_grp_base_year_table = p.gep_by_income_grp_base_year_table[['income_grp', 'Value']]
    ...
    if k == 'gep_by_year':
        p.gep_by_year_table = df[['year', 'terrestrial_carbon_gep']]
        # Drop values that are zero
        p.gep_by_year_table = p.gep_by_year_table[p.gep_by_year_table['terrestrial_carbon_gep'] > 0]
        ...
        plt.plot(p.gep_by_year_table['year'], p.gep_by_year_table['terrestrial_carbon_gep'], marker='o')
...
The analysis reveals that commercial agriculture generates a total gross economic product of $`{python} f"{total_terrestrial_carbon_value:,.2f}"` across the study period. Below we present the results in various aggregations, ending with the GEP by country in the base year.
