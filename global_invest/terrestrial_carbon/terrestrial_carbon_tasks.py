...
def task_combine_two_carbon_density_maps(p):
    ...
    # Run the function
    terrestrial_carbon_functions.combine_two_float_rasters(
...
def task_reproject_total_carbon_density(p):
    ...
    # Input and output paths
    p.total_terrestrial_carbon_density_path = p.get_path(os.path.join(p.project_dir, "total_biomass_carbon_2010_float.tif"))
    p.reprojected_total_terrestrial_carbon_density_path = os.path.join(p.project_dir, "total_biomass_carbon_2010_float_reprojected.tif")

    # Run the function
    terrestrial_carbon_functions.reproject_raster(
        input_path=p.total_terrestrial_carbon_density_path,
        reference_path=p.base_year_lulc_path,
        output_path=p.reprojected_total_terrestrial_carbon_density_path,
        compress="lzw",
...
def task_reproject_terrestrial_carbon_zones(p):
    """
    Task to reproject the total carbon density raster to the project's coordinate reference system (CRS).
    """

    # Input and output paths
    p.reprojected_terrestrial_carbon_zones_path = os.path.join(p.project_dir, "carbon_zones_rasterized_reprojected.tif")

    # Run the function
    terrestrial_carbon_functions.reproject_raster(
        input_path=p.terrestrial_carbon_zones_path,
        reference_path=p.base_year_lulc_path,
        output_path=p.reprojected_terrestrial_carbon_zones_path,
        compress="lzw",
        chunks={"x": 1024, "y": 1024},
        overwrite=False
        )
...
def task_compute_terrestrial_carbon_density_table(p):

    p.reprojected_total_terrestrial_carbon_density_path = p.get_path(os.path.join(p.project_dir, "total_biomass_carbon_2010_float_reprojected.tif"))
    p.terrestrial_carbon_density_lookup_table_path = os.path.join(p.project_dir, "carbon_density_lookup_table.csv")

    terrestrial_carbon_functions.stack_layers_to_csv(
        group_layer1_path=p.base_year_lulc_path,
        group_layer2_path=p.terrestrial_carbon_zones_path,
        value_layer_path=p.reprojected_total_terrestrial_carbon_density_path,
        output_path=p.terrestrial_carbon_density_lookup_table_path,
        group1_name="lulc_id",
...
def task_generate_terrestrial_carbon_density_raster_base_year(p):
    p.reprojected_total_terrestrial_carbon_density_path = p.get_path(os.path.join(p.project_dir, "total_biomass_carbon_2010_float_reprojected.tif"))
    p.terrestrial_carbon_density_lookup_table_path = p.get_path(os.path.join(p.project_dir, "carbon_density_lookup_table.csv"))
    p.terrestrial_carbon_density_raster_output_path = os.path.join(p.project_dir, "carbon_density_2019.tif")
    terrestrial_carbon_functions.generate_terrestrial_carbon_density_raster(
        ...
        out_path=p.terrestrial_carbon_density_raster_output_path)
    return True


def task_summarize_terrestrial_carbon_density_by_region(p):
    p.terrestrial_carbon_density_raster_output_path = p.get_path(os.path.join(p.project_dir, "carbon_density_2019.tif"))
    ...
    result = terrestrial_carbon_functions.summarize_raster_by_region(
        value_raster_path=p.terrestrial_carbon_density_raster_output_path,
