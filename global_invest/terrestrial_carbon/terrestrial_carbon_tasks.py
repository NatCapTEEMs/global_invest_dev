...
def task_reproject_total_carbon_density(p):
    ...
    p.total_carbon_density_path = getattr(p, 'total_carbon_density_path', None) or p.get_path('global_invest', 'carbon', 'spawn_total_biomass_carbon_2010.tif')
...
def task_compute_terrestrial_carbon_shock(p):
    ...
    if not getattr(p, 'terrestrial_carbon_zones_path', None):
        p.terrestrial_carbon_zones_path = p.get_path('global_invest', 'carbon', 'carbon_zones_rasterized.tif')
    if not getattr(p, 'terrestrial_carbon_density_lookup_table_path', None):
        p.terrestrial_carbon_density_lookup_table_path = p.get_path('global_invest', 'carbon', 'carbon_density_lookup_seals7_spawn.csv')
