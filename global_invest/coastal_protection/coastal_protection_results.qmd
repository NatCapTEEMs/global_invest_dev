...
for k, v in p.results[service_label].items():
    ...
    if k == 'gep_by_country_base_year':
        ...
        p.gep_by_country_base_year_table = df[['iso3_r250_name', 'Value']]

        # Group by useful aggregation columns explicitly and assign each as an attribute to p
        if 'income_grp' in df.columns:
            p.gep_by_income_grp_base_year_table = df.groupby('income_grp', as_index=False)['Value'].sum()
            p.gep_by_income_grp_base_year_table = p.gep_by_income_grp_base_year_table[['income_grp', 'Value']]
        if 'region_un' in df.columns:
            p.gep_by_region_un_base_year_table = df.groupby('region_un', as_index=False)['Value'].sum()
            p.gep_by_region_un_base_year_table = p.gep_by_region_un_base_year_table[['region_un', 'Value']]
        if 'continent' in df.columns:
            p.gep_by_continent_base_year_table = df.groupby('continent', as_index=False)['Value'].sum()
            p.gep_by_continent_base_year_table = p.gep_by_continent_base_year_table[['continent', 'Value']]
        if 'subregion' in df.columns:
            p.gep_by_subregion_base_year_table = df.groupby('subregion', as_index=False)['Value'].sum()

            # p.gep_by_subregion_base_year_table = p.gep_by_country_base_year_table[['subregion', 'Value']]
        ...
        gdf = gdf[['iso3_r250_id', 'iso3_r250_name', 'Value', 'geometry']]  # Keep only relevant columns
        ...
        norm = mpl.colors.LogNorm(vmin=gdf['Value'][gdf['Value'] > 0].min(), vmax=gdf['Value'].max())

        fig, ax = plt.subplots(figsize=(15, 10))
        gdf.plot(column='Value', cmap='OrRd', legend=False, norm=norm, ax=ax)
