...
Storm damage avoided by mangroves and coral reefs is valued at $`{python} f"{total_value:,.2f}"` in the base year. Both source tables already carry a final per-country value, so this page reports them rather than a chain we ran. The tables below aggregate by income group, region and continent.
