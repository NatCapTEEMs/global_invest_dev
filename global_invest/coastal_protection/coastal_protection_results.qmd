...
for k, v in p.results[service_label].items():
    ...
    if k == 'gep_by_country_base_year':
        value_column = 'coastal_protection_gep'
        total_value = df[value_column].sum()
        p.gep_by_country_base_year_table = df[['iso3_r250_name', value_column]]

        # Group by useful aggregation columns explicitly and assign each as an attribute to p
        if 'income_grp' in df.columns:
            p.gep_by_income_grp_base_year_table = df.groupby('income_grp', as_index=False)[value_column].sum()
            p.gep_by_income_grp_base_year_table = p.gep_by_income_grp_base_year_table[['income_grp', value_column]]
        if 'region_un' in df.columns:
            p.gep_by_region_un_base_year_table = df.groupby('region_un', as_index=False)[value_column].sum()
            p.gep_by_region_un_base_year_table = p.gep_by_region_un_base_year_table[['region_un', value_column]]
        if 'continent' in df.columns:
            p.gep_by_continent_base_year_table = df.groupby('continent', as_index=False)[value_column].sum()
            p.gep_by_continent_base_year_table = p.gep_by_continent_base_year_table[['continent', value_column]]
        if 'subregion' in df.columns:
            p.gep_by_subregion_base_year_table = df.groupby('subregion', as_index=False)[value_column].sum()
        ...
        gdf = gdf[['iso3_r250_id', 'iso3_r250_name', value_column, 'geometry']]  # Keep only relevant columns
        ...
        norm = mpl.colors.LogNorm(vmin=gdf[value_column][gdf[value_column] > 0].min(),
                                  vmax=gdf[value_column].max())

        fig, ax = plt.subplots(figsize=(15, 10))
        gdf.plot(column=value_column, cmap='OrRd', legend=False, norm=norm, ax=ax)
...
Storm damage avoided by mangroves and coral reefs is valued at $`{python} f"{total_value:,.2f}"` in the base year. Both source tables already carry a final per-country value, so this page reports them rather than a calculation we ran. The tables below aggregate by income group, region and continent.
