...
from global_invest.coastal_protection import coastal_protection_defaults
...
for k, v in p.results[service_label].items():
    ...
    if k == 'gep_by_country_base_year':
        total_agricultural_value = df['coastal_protection_gep'].sum()
        p.gep_by_country_base_year_table = df[['iso3_r250_name', 'coastal_protection_gep']]

        # Group by useful aggregation columns explicitly and assign each as an attribute to p
        if 'income_grp' in df.columns:
            p.gep_by_income_grp_base_year_table = df.groupby('income_grp', as_index=False)['coastal_protection_gep'].sum()
            p.gep_by_income_grp_base_year_table = p.gep_by_income_grp_base_year_table[['income_grp', 'Value']]
        if 'region_un' in df.columns:
            p.gep_by_region_un_base_year_table = df.groupby('region_un', as_index=False)['coastal_protection_gep'].sum()
            p.gep_by_region_un_base_year_table = p.gep_by_region_un_base_year_table[['region_un', 'coastal_protection_gep']]
        if 'continent' in df.columns:
            p.gep_by_continent_base_year_table = df.groupby('continent', as_index=False)['coastal_protection_gep'].sum()
            p.gep_by_continent_base_year_table = p.gep_by_continent_base_year_table[['continent', 'coastal_protection_gep']]
        if 'subregion' in df.columns:
            p.gep_by_subregion_base_year_table = df.groupby('subregion', as_index=False)['coastal_protection_gep'].sum()

            # p.gep_by_subregion_base_year_table = p.gep_by_country_base_year_table[['subregion', 'coastal_protection_gep']]
        ...
        gdf = gdf[['iso3_r250_id', 'iso3_r250_name', 'coastal_protection_gep', 'geometry']]  # Keep only relevant columns
        ...
        norm = mpl.colors.LogNorm(vmin=gdf['coastal_protection_gep'][gdf['coastal_protection_gep'] > 0].min(), vmax=gdf['coastal_protection_gep'].max())

        fig, ax = plt.subplots(figsize=(15, 10))
        gdf.plot(column='coastal_protection_gep', cmap='OrRd', legend=False, norm=norm, ax=ax)
    ...
    if k == 'gep_by_year':
        p.gep_by_year_table = df[['year', 'coastal_protection_gep']]
        # Drop values that are zero
        p.gep_by_year_table = p.gep_by_year_table[p.gep_by_year_table['coastal_protection_gep'] > 0]
        ...
        plt.plot(p.gep_by_year_table['year'], p.gep_by_year_table['coastal_protection_gep'], marker='o')
