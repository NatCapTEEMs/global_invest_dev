...
def merge_crop_with_coefs(df_crop_value: pd.DataFrame, df_crop_coefs: pd.DataFrame):
    ...
    df_crop_value["crop_provision_gep"] = df_crop_value["crop_provision_gep"] * df_crop_value["rental_rate"]
...
def group_crops(df: pd.DataFrame):
    ...
    # # OLD METHOD
    # df_gep_by_year_country = df.groupby(["area_code", "ee_r264_id", "ee_r264_label", "country", "year"], as_index=False).agg(crop_provision_gep=("crop_provision_gep", "sum"))    
    # df_gep_by_year_country = df_gep_by_year_country.sort_values(by=["area_code", "year"], ascending=[True, True])
    # df_gep_by_year_country["crop_provision_gep"] = pd.to_numeric(df_gep_by_year_country["crop_provision_gep"], errors="coerce")
    
    df_gep_by_year_country = hb.df_groupby(df, groupby_cols, agg_dict={"crop_provision_gep": "sum"}, preserve='keep_all_valid') 
...
def group_countries(df: pd.DataFrame):
    ...
    df_gep_by_year = hb.df_groupby(df, groupby_cols='year', agg_cols="crop_provision_gep", preserve='keep_all_valid')

    
    # START HERE: df_gep_by_year = hb.df_groupby(df, groupby_cols='iso3_r250_label', agg_dict={"crop_provision_gep": "sum"}). This line causes a really wrongly formatted DataFrame.
...
# def calculate_gep(p, data_input_dir, items: list = crop_provision_tasks.DEFAULT_CROP_ITEMS, base_year: int = 2019):
...
#     total_value = df_gep_by_year[df_gep_by_year["year"] == base_year]["crop_provision_gep"].sum()
